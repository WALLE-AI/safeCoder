...
export class CodeShellWebviewViewProvider implements vscode.WebviewViewProvider {
	public static readonly viewId = "safecoder.chatView";
	...
	public async executeQuickFixBugCommand(command: string,message: string, code: string, edit: boolean){
		...
		let humanPrompt = prompt.createPromptQuickFixBug(languageId, message,code);

		// focus gpt activity from activity bar
		if (!this._view) {
			await vscode.commands.executeCommand("safecoder.chatView.focus");
		} else {
			this._view?.show?.(true);
		}
		//函数在这里执行没有历史记录的数据，采用单轮策略，避免受到上下文的影响，特别是多语言交叉使用时候
		this.startQuestion(humanPrompt,command);
	 ...
	 * @param question ��问内容
	 */
	private startQuestion(question: string,command:string="") {
		const contentIndex = this.sessionItem.chatList.length;
		const divContent = this._makeQuestionAnswerDiv(contentIndex); // 创建提问回答的HTML元素
		const eventData = {
			"divContent": divContent,
			"contentIndex": contentIndex,
			"question": question
		}; // 发送事件数据，包含提问回答的HTML元素和相关信息
		this._view?.webview.postMessage({ type: "addQuestionAnswerDiv", value: eventData });

		const humanMessage = new HumanMessage(question); // 创建人类消息对象
		const aiMessage = new AIMessage(""); // 创建AI消息对象
		const chatItem = new ChatItem(humanMessage, aiMessage); // 创建聊天记录对象
		this.sessionItem.addChatItem(chatItem); // 将聊天记录添加到会话中
		this.sessionItem.setHistoryMsg(question)
		this.generateAnswer(contentIndex,command); // 生成回答
	}

	//解析该json数据中choice字段的数据{"id": "chatcmpl-0d824767-7b52-4cde-ae11-a37dbe946827", "model": "/home/models/phind/Phind-CodeLlama-34B-v2-GGUF/phind-codellama-34b-v2.Q4_0.gguf", "created": 1698302921, "object": "chat.completion.chunk", "choices": [{"index": 0, "delta": {"content": "\u4f5c"}
	private parseChoiceData(jsonData: any): string {
		...
		for (let i = 0; i < choices.length; i++) {
			...
			if (delta.hasOwnProperty("content")){
				choiceStr = choice.delta.content
			}
		}
		return choiceStr;
	}
	
	private generateAnswer(index: number,command:string="") {
		const chatItem = this.sessionItem.chatList[index];
		chatItem.aiMessage.content = "";
		this.sessionStore.update(this.sessionItem);
		let inputMsg = new Array<{} | undefined>();
		const historyCodeShellPrompt = this.sessionItem.getSlicePrompt(0, index);
		const respData = {
			"contentIndex": index,
			"responseText": chatItem.aiMessage.content,
			"aiMsgId": chatItem.aiMsgId,
		};
		//如何cmmand参数不为""，执行currenMsg逻辑
		//command不为""，执行currenMsg逻辑	
		if(command !== ""){
			let currentMsg = this.sessionItem.getCurrentMsg()
			console.log("currentMsg:", currentMsg);
			inputMsg = currentMsg

		} else {
			let historyLLamacppPromptMsg = this.sessionItem.getHistoryMsg()
			console.log("historyLLamacppPromptMsg",historyLLamacppPromptMsg)
			inputMsg = historyLLamacppPromptMsg
		}

		console.log("historyCodeShellPrompt:", historyCodeShellPrompt);
		postEventStream(inputMsg, (data) => {
			let content:string  = ""
			let jsonData:any
			//Todo：这个地方的逻辑有点奇怪
			if (data != "[DONE]"){
				jsonData = JSON.parse(data);
				content= this.parseChoiceData(jsonData)
				if (content) {
					//jsonData.content转成utf-8格式 
					chatItem.aiMessage.append(content);
				}
			...
			console.log("chatItem.aiMessage.content;",chatItem.aiMessage.content)
			...
			this.sessionStore.update(this.sessionItem);
			//TODO:优化该异常处理
			if(_err?.type !== 'aborted'){
				respData.responseText = _err.stack;
				this._view?.webview.postMessage({ type: "addStreamResponse", value: respData });
			}
			this._view?.webview.postMessage({ type: "responseStreamDone", value: respData });
		});
	...
	private _makeQuestionAnswerDiv(contentIndex: number) {
		...
		<div id="qa_section_div_${contentIndex}"
			...
			<div class="question-container col-12">
				<div class="row mr-0">
					<div class="col-4 pr-0"><div class="question-title">You</div></div>
			...
			<div class="row mr-0">
				<div class="col-4 pr-0">
					<div class="answer-title">safeCoder</div>
				</div>
				<div class="col-8 p-0" style="align-items: center; display: flex; justify-content: flex-end;">
					...
					<div _ngcontent-uim-c116 role="banner" class="toolbar bc-header-fixed">
						<div _ngcontent-uim-c116 id="navigation" class="d-flex position-relative feed-sect">
							<a _ngcontent-uim-c116 id="feedTab" routerlinkactive="active" routerlink="chat" tabindex="-1"
								...
								<svg t="1705240682386" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="4253" width="20" height="20"><path d="M409.6 614.4a40.96 40.96 0 1 0 0 81.92 40.96 40.96 0 0 0 0-81.92z m-286.72-40.96a40.96 40.96 0 0 0-40.96 40.96v81.92a40.96 40.96 0 1 0 81.92 0v-81.92a40.96 40.96 0 0 0-40.96-40.96z m819.2 0a40.96 40.96 0 0 0-40.96 40.96v81.92a40.96 40.96 0 1 0 81.92 0v-81.92a40.96 40.96 0 0 0-40.96-40.96z m-204.8-286.72h-163.84V234.2912A81.92 81.92 0 0 0 614.4 163.84a81.92 81.92 0 1 0-163.84 0 81.92 81.92 0 0 0 40.96 70.4512V286.72H327.68a122.88 122.88 0 0 0-122.88 122.88v368.64a122.88 122.88 0 0 0 122.88 122.88h409.6a122.88 122.88 0 0 0 122.88-122.88v-368.64a122.88 122.88 0 0 0-122.88-122.88z m-134.3488 81.92l-20.48 81.92h-99.9424l-20.48-81.92h140.9024zM778.24 778.24a40.96 40.96 0 0 1-40.96 40.96H327.68a40.96 40.96 0 0 1-40.96-40.96v-368.64a40.96 40.96 0 0 1 40.96-40.96h49.9712l31.9488 132.7104a40.96 40.96 0 0 0 40.96 31.1296h163.84a40.96 40.96 0 0 0 40.96-31.1296L687.3088 368.64H737.28a40.96 40.96 0 0 1 40.96 40.96v368.64z m-122.88-163.84a40.96 40.96 0 1 0 0 81.92 40.96 40.96 0 0 0 0-81.92z" fill="#cdcdcd" p-id="4254"></path></svg>
								<span _ngcontent-uim-c116 class="ml-2 hd-low-res"> Chat </span>
							</a>
						</div>
										...
										<div class="history-btn-section row">
											<div class="col-6 d-flex justify-content-start">
												<div tabindex="0" id="historyBackButton" class="history-btn focus-on-tab">
													...
													<span class="ml-2 history-buttons">Return</span>
												</div>
											</div>
										</div>
										...
										<div class="history-btn-section row">
											<div class="col-6 d-flex justify-content-start">
												<div tabindex="0" aria-label="history" id="historyButton" class="history-btn focus-on-tab">
													...
													<span class="ml-2 history-buttons">History</span>
												</div>
											</div>
											...
											<div class="tzs-common-textarea-section">
												<div class="fake-input textarea-container pb-2"
													...
													<textarea nz-input aria-label="ai-input-box"
														...
														placeholder="Please Ask question, Enter messages" style="height: 35px; overflow: hidden;"></textarea>
													<div id="reply-buttons2" class="reply-buttons2"
														style="overflow: hidden;">
														<div class="row">
															<div class="col-3 text-left d-flex"></div>
															<div class="col-9 text-right">
																<span class="shift-enter-text"><span style="font-weight: 600;">Shift + Enter</span> </span>
