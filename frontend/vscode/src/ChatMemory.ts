...
export class SessionItem {
    ...
    //模型的系统角色暴露在client
    historyMsgList:Array<{}> = [{'role':"system","content":"我是智能编码助手-safeCoder,由walle-ai team开发。"}];
    ...
    setHistoryMsg(question:string){
        let historyMsgDict = {"role":"user","content":""};
        historyMsgDict['content'] = question
        this.historyMsgList.push(historyMsgDict)
    }
    //获取当前msg信息
    getCurrentMsg():Array<{} | undefined>{
        let currentMsgList = new Array<{} | undefined>()
        currentMsgList.push(this.historyMsgList[this.historyMsgList.length-1])
