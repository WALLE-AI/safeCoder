...
export class TabbyStatusBarItem {
  ...
  constructor(completionProvider: TabbyCompletionProvider) {
    ...
    agent().on("statusChanged", (event) => {
      console.debug("safecoder agent statusChanged", { event });
      this.fsmService.send(event.status);
    });

    agent().on("authRequired", (event) => {
      console.debug("safecoder agent authRequired", { event });
    ...
    agent().on("newIssue", (event) => {
      console.debug("safecoder agent newIssue", { event });
  ...
  private toInitializing() {
    ...
    this.item.tooltip = "safecoder is initializing.";
    this.item.command = {
      title: "",
  ...
  private toAutomatic() {
    ...
    this.item.tooltip = "safecoder automatic code completion is enabled.";
    this.item.command = {
      title: "",
      command: "safecoder.applyCallback",
  ...
  private toManual() {
    ...
    this.item.tooltip = "safecoder is standing by, click or press `Alt + \\` to trigger code completion.";
    this.item.command = {
      title: "",
      command: "safecoder.applyCallback",
  ...
  private toLoading() {
    ...
    this.item.backgroundColor = backgroundColorNormal;
    this.item.text = `${iconLoading} ${label}`;
    this.item.tooltip = "safecoder is generating code completions.";
    this.item.command = {
      title: "",
      command: "safecoder.applyCallback",
  ...
  private toDisabled() {
    ...
    this.item.tooltip = "safecoder is disabled. Click to check settings.";
    this.item.command = {
      title: "",
      command: "safecoder.applyCallback",
    ...
    console.debug("safecoder code completion is enabled but inline suggest is disabled.");
  ...
  private toDisconnected() {
    ...
    this.item.tooltip = "Cannot connect to safecoder Server. Click to open settings.";
    this.item.command = {
      title: "",
      command: "safecoder.applyCallback",
  ...
  private toUnauthorized() {
    ...
    this.item.tooltip = "safecoder Server requires authorization. Click to continue.";
    this.item.command = {
      title: "",
      command: "safecoder.applyCallback",
      arguments: [
